---
name: kubesphere
description: KubeSphere 4.1 多集群与 IAM 运维技能。默认流程是最小授权开户：用户名=workspace=各子集群 namespace，global role 固定为 platform-regular，仅绑定指定 clusters，并在同名 namespace 下授 admin。涉及这条默认开户流程或其排障时必须使用。
---
...
2. 默认流程固定为：
   ...
   - 只给指定 clusters
3. 默认不给 `cluster-viewer` 或 `cluster-admin`，除非用户明确要求。
...
2. `GlobalRoleBinding`
3. `WorkspaceTemplate`
4. `WorkspaceRoleBinding`
5. `Namespace`
6. namespace 下的 `RoleBinding.iam.kubesphere.io`
