...
- global role = platform-regular  (no cluster-viewer)
...
def build_namespace_patch(username: str) -> dict:
    return {
        "apiVersion": "v1",
        "kind": "Namespace",
        "metadata": {
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--email",
        default=None,
        help="User email. Defaults to <username>@example.com",
    )
    parser.add_argument(
        "--password",
        default=None,
        help="Initial plaintext password. Defaults to <username>@123KS",
    )
    parser.add_argument(
        "--cluster",
        action="append",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Skip rendering the Namespace label patch document. "
...
def validate_namespaces_exist(args: argparse.Namespace) -> None:
    ...
    for cluster in args.clusters:
        ...
        namespace = kubectl_get_json(context, ["namespaces"], args.username)
        if namespace is None:
            missing.append(f"  {cluster} ({context}): ns/{args.username} not found")
    if missing:
        details = "\n".join(missing)
        raise SystemExit(
            "refusing to render: the following namespaces do not exist yet.\n"
            "Please create them first, then re-run this script.\n"
            f"{details}"
        )
...
def main() -> int:
    ...
    if not args.no_namespace_labels:
        documents.insert(-1, build_namespace_patch(args.username))
