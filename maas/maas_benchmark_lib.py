#!/usr/bin/env python3
"""
Shared benchmark primitives for Maas vLLM testing.

Key design decisions (learned from production benchmarking):
- Always use ingress URL, never port-forward (drops connections at 16+ concurrency)
- Default 24 concurrency / 256 output tokens for meaningful throughput measurement
- Auto-detect model from /v1/models endpoint when not specified
- Sustained mode with clean live reporting
"""
...
def build_default_thanos_queries(
    ...
    # DCU metrics use dcu_pod_name/dcu_pod_namespace labels (from dcu-exporter)
...
def resolve_model(args: argparse.Namespace, base_url: str) -> str:
    """Resolve model: use --model if given, otherwise auto-detect from /v1/models."""
    if args.model:
        return args.model
